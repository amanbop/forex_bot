                ...
                {                
                    ...
                    if ((mHasAccount) && (mHasOffer)) {
                        ...
                        if (requestFactory != null) {
                            ...
                            System.out.println("Sending request to create order...");
                            mSession.sendRequest(request);
                            
                            tableListener.WaitForOrderCreation();
                            if (!responseListener.hasError()) 
                            {
                                tableManager.lockUpdates();                            
                                try
                                {
                                    for(int ii=0; ii<ordersTable.size();ii++)
                                    {
                                        O2GOrderTableRow order = ordersTable.getRow(ii);
