...
void EventListener::waitNextEvent()
{
    ...
    while (bContinue)
    {
        DWORD dwRes = uni::WaitForMultipleObjects(4, aHandles, FALSE, INFINITE);
...
void EventListener::onResponse()
{
    ...
    while (response)
    {
        ...
        else if (response->getType() == CommandResponse)
    ...
    for (int i = 0; i < genericReader->size(); i++)
    {
        ...
        for (int j = 0; j < count; j++)
        {
            ...
            case IO2GTableColumn::Boolean:
            {
                ...
                stream << column->getID() << "=" << boolToString(bValue) << " Valid=" << sIsValid << " ";
            }
            break;
...
void EventListener::printToLogOffer(IO2GOfferRow *offer)
{
    ...
    sprintf_s(szBuffer, sizeof(szBuffer), "%s OfferID=%s Bid=%f Ask=%f Low=%f High=%f Volume=%i QuoteID=%s Time=%s BidTradable=%s AskTradable=%s ContractCurrency=%s Digits=%i PointSize=%f SubscriptionStatus=%s TradingStatus=%s InstrumentType=%i ContractMultiplier=%f ValueDate=%s BuyInterest=%f SellInterest=%f",
                                            offer->getInstrument(),
                                            offer->getOfferID(),
                                            offer->getBid(),
                                            offer->getAsk(),
                                            ...
                                            offer->getValueDate(),
                                            offer->getBuyInterest(),
                                            offer->getSellInterest());
...
void EventListener::printToLogAccount(IO2GAccountRow *account)
{
    ...
    std::string maintenanceFlag = boolToString(account->getMaintenanceFlag());
...
void EventListener::printToLogOrder(IO2GOrderRow *order)
{
    ...
    std::string netQuantity = boolToString(order->getNetQuantity());
    std::string workingIndicator = boolToString(order->getWorkingIndicator());
...
void EventListener::onCommandResponse(IO2GResponse *response)
{
    mOrderSample->onCommandResponse(response);
}
