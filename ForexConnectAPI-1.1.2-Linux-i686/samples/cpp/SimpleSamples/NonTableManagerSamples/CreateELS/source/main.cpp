...
int main(int argc, char *argv[])
{
    ...
    if (connected)
    {
        ...
        if ( bOffer && bAccount )
        {
            ...
            std::string sOrderType = O2G2::Orders::StopEntry;
            ...
            // and attach stop and limit orders 50 pips from an entry order price
            if (sOrderType == O2G2::Orders::LimitEntry)
            {
                if (sBuySell == O2G2::Buy)
                {
                    dRate = dAsk - 8 * dPointSize;
                    dRateLimit = dRate + 50 * dPointSize;
                    dRateStop = dRate - 50 * dPointSize;
                }
                else
                {
            ...
            valuemap->setString(OrderType, sOrderType.c_str());
