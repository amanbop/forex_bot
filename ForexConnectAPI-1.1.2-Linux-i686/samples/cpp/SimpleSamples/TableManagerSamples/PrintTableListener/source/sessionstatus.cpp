...
/** Constcructor*/
TablesManagerStatusListener::TablesManagerStatusListener()
{
    mNotifyEvent = uni::CreateEvent(NULL, FALSE, FALSE, 0);
    dwRef = 1;
    mLoaded = false;
}
...
TablesManagerStatusListener::~TablesManagerStatusListener()
{
    uni::CloseHandle(mNotifyEvent);
}

/** Status change*/
void TablesManagerStatusListener::onStatusChanged(O2GTableManagerStatus status, IO2GTableManager *manager)
{
    if (status == TablesLoaded || status == TablesLoadFailed)
    {

        mLoaded = (status == TablesLoaded);
        uni::SetEvent(mNotifyEvent);
    }
}

/** Gets load event.*/
HANDLE TablesManagerStatusListener::getTablesLoadedEvent()
{
    return  mNotifyEvent;
}

long TablesManagerStatusListener::addRef()
{
    return InterlockedIncrement(&dwRef);
}

long TablesManagerStatusListener::release()
{
...
bool TablesManagerStatusListener::isTablesLoaded()
{
    return mLoaded;
}
