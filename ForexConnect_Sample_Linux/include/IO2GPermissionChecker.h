...
class IO2GPermissionChecker : public IAddRef
{
 ...
 public:
    ...
    /** Whether it is possible to add new orders to the existing OCO. */
    virtual O2GPermissionStatus canJoinOCO(const char* instrument) = 0;

    /** Whether it is possible to exclude orders from the existing OCO. */
    virtual O2GPermissionStatus canExcludeOCO(const char* instrument) = 0;
