...
void TableListener::setRequest(const char *sRequestID)
{
    mOrdersCreated = 0;
...
void TableListener::onAdded(const char *rowID, IO2GRow *row)
{
    if (row->getTableType() == Orders)
    {
        ...
        if (mRequestID == orderRow->getRequestID())
        {
            ...
            std::cout << "TableListener: Added order. ID: " << orderRow->getOrderID() << \
                " (" << sOrderType << ")" << std::endl;
            mOrdersCreated++;
            if (mOrdersCreated == 3) // entry + limit + stop orders
                uni::SetEvent(mEvent);
        }
    }
}
